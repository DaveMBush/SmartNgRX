...
type GenericChildDefinition = ChildField & Omit<ChildDefinition<object>, 'childField'>;
...
class ChildDefinitionRegistry {
  ...
  registerChildDefinition<P>(
    ...
    this.childDefinitionMap.set(`${feature}${psi}${entity}`, castTo<GenericChildDefinition>(childDefinition));
  }
   ...
   */
  getChildDefinition<P>(
    feature: string,
    entity: string,
  ): ChildDefinition<P> {
    const childDefinition = this.childDefinitionMap.get(`${feature}${psi}${entity}`);
    assert(!!childDefinition, `Child definition not found for feature: ${feature} and entity: ${entity}`);
