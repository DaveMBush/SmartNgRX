...
export interface MarkAndDeleteInit {
   ...
   * re-retrieve the row if `markDirtyFetchesNew` is true. If
   * `markDirtyFetchesNew` is false, it will just reset the isDirty
   * flag in the row.  his is how we know the row is still in use,
   * or not.
   *
   * If `markDirtyTime` is set to -1, then the row will never be
   * marked as dirty, and `removeTime` will be ignored. If you manually
   * mark a row as dirty (coming soon), the system will assume you want
   * to refetch the row from the server, regardless of what you set here.
   * This allows us to use the same mechanism to refresh data for automated
   ...
   * store. `removeTime` must be greater than `markDirtyTime`. If it is
   * not, it will automatically set to twice `markDirtyTime`.
   */
  removeTime: number;
  /**
