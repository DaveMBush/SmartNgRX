...
import { selectCurrentLocationId } from './select-current-location.selectors';

function locationEqualsLocationIdFunction(id: string) {
  return function locationEqualsLocationIdFunction2(l: string): boolean {
    return l === id;
  };
}
  ...
  function watchLocationsFunction(
    ...
    return actions.pipe(
      ...
      filter(function watchLocationsFilterFunction({ ids, locationId }) {
        return !ids.some(locationEqualsLocationIdFunction(locationId));
      }),
